...
const std::array<bool, 4>& GameState::getMilitaryTokensStatus() const
{
	return m_militaryTokens;
}

bool GameState::removeMilitaryToken(int index)
{
	if (index >= 0 && index < 4 && m_militaryTokens[index]) {
		m_militaryTokens[index] = false;
		return true;
	}
	return false;
}
