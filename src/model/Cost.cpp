...
Cost::Cost(uint8_t coins) : m_coinCost(coins) {}
Cost::Cost(uint8_t coins, const std::map<ResourceType, uint8_t>& resources)
	: m_coinCost(coins), m_resourceCosts(resources) {
}

Cost& Cost::withCoinCost(uint8_t coins)
{
	m_coinCost = coins;
	return *this;
}

Cost& Cost::withResourceCost(ResourceType type, uint8_t amount)
{
	m_resourceCosts[type] = amount;
	return *this;
}

uint8_t Cost::getCoinCost() const {
	return m_coinCost;
}

const std::map<ResourceType, uint8_t>& Cost::getResourceCosts() const {
	return m_resourceCosts;
}

uint8_t Cost::getResourceCost(ResourceType type) const {
...
std::string Cost::toString() const {
	...
	std::stringstream ss;
	if (m_coinCost > 0) {
		ss << static_cast<int>(m_coinCost) << " coin";
		if (m_coinCost > 1)
			ss << "s";
	}
	if (!m_resourceCosts.empty()) {
		...
		for (const auto& [type, quantity] : m_resourceCosts) {
			if (!first)
				ss << " + ";
			ss << static_cast<int>(quantity) << " " << resourceToString(type);
			if (quantity > 1)
				ss << "s";
			first = false;
		}
...
std::string Cost::toShortString() const {
	...
	std::stringstream ss;
	if (m_coinCost > 0) {
		ss << static_cast<int>(m_coinCost) << "G";
	}
	if (!m_resourceCosts.empty()) {
		if (m_coinCost > 0)
			ss << "+";
...
Cost Cost::applyGeneralDiscount(int amount, const std::vector<ResourceType>& preferredOrder) const {
	...
	for (ResourceType type : preferredOrder) {
		...
		auto it = result.m_resourceCosts.find(type);
		if (it != result.m_resourceCosts.end()) {
			int canDiscount = std::min(remainingDiscount, static_cast<int>(it->second));
...
void from_json(const json& j, Cost& cost)
{
	...
	cost.m_resourceCosts = j.value("resourceCosts", std::map<ResourceType, uint8_t>{});
}
